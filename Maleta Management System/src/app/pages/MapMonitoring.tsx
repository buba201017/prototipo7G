...
import { MapPin, Package, Plane, RefreshCw, ZoomIn, ZoomOut, PlayCircle, Settings2, Database } from 'lucide-react';
...
export function MapMonitoring() {
  ...
  const [simulationResult, setSimulationResult] = useState<{ status: string, time: string, delivered: number } | null>(null);
    ...
    <div className="relative w-full h-[calc(100vh-2rem)] min-h-[600px] bg-[#dcf1fb] overflow-hidden rounded-xl border border-gray-200 flex shadow-sm">
      ...
      >
        <g transform={`translate(${transform.x}, ${transform.y}) scale(${transform.k})`}>
          ...
          {filteredAirports.map((airport) => {
            ...
            const hasActive = activeLuggage.some(l => l.currentLocationId === airport.id);
            const color = hasActive ? '#ef4444' : '#3b82f6';
      ...
      <div className="absolute inset-0 pointer-events-none p-4 md:p-6 flex justify-between">
        ...
        <div className="flex flex-col gap-4 pointer-events-none">
          {/* Top-Left Date Badge */}
          <div className="bg-[#0275d8] text-white px-4 py-2 rounded-md shadow-md text-sm font-semibold max-w-max pointer-events-auto">
            24/08/2025 - 00:00:00
          </div>
          ...
          <div className="mt-auto pointer-events-auto bg-white/95 backdrop-blur-md border border-gray-200 shadow-md rounded-lg p-3 space-y-2 w-max">
             <div className="text-gray-800 font-semibold mb-2 text-sm">Leyenda</div>
             <div className="flex items-center gap-2 text-gray-600 text-xs">
               <div className="w-3 h-3 rounded-full bg-[#3b82f6] border border-white shadow-sm shrink-0" />
               <span>Aeropuerto</span>
             </div>
             <div className="flex items-center gap-2 text-gray-600 text-xs">
               <div className="w-3 h-3 rounded-full bg-[#ef4444] border border-white shadow-sm shrink-0" />
               <span>Con maletas activas</span>
             </div>
             <div className="flex items-center gap-2 text-gray-600 text-xs">
        ...
        <div className="flex gap-4 items-start max-h-full">
          ...
          {activePanel && (
            <div className="w-[360px] pointer-events-auto flex flex-col hide-scrollbar" style={{ maxHeight: '100%' }}>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-col gap-2.5">
                        <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                          <input type="radio" name="sim_type" defaultChecked className="w-4 h-4 text-blue-600 focus:ring-blue-500" /> 
                          Semanal
                        </label>
                        <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                          <input type="radio" name="sim_type" className="w-4 h-4 text-blue-600 focus:ring-blue-500" /> 
                          Hasta el colapso
                        </label>
                      </div>
                    </div>
                      ...
                      <div className="mt-4 pt-5 border-t border-gray-100 space-y-3.5" style={{ animation: 'fadeIn 0.5s ease' }}>
                         ...
                         <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3.5 space-y-3">
                            <div className="flex justify-between items-center text-sm">
                               ...
                               <Badge className="bg-green-100 text-green-700 hover:bg-green-100 border-0">{simulationResult.status}</Badge>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                            ...
                            <div className="flex justify-between items-center text-sm">
                               ...
                               <span className="font-semibold text-[#0d6efd]">{simulationResult.delivered.toLocaleString()}</span>
                            </div>
                         </div>
                      </div>
                    )}
              ...
              {activePanel === 'aeropuertos' && (
                <Card className="shadow-2xl border-0 bg-white/95 backdrop-blur-md flex flex-col overflow-y-auto max-h-full hide-scrollbar shrink-0">
                  ...
                  <CardContent className="pt-3 flex-1 pb-4">
                    <div className="space-y-2.5">
                      {filteredAirports.map(airport => {
                        const pct = (airport.currentStorage / airport.storageCapacity) * 100;
